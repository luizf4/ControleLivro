...
public class EmprestimoDAO {
    ...
    public List<Emprestimo> consultarDevolvidos() throws Exception {
        ...
        try {
            String consultarDevolvidos = "select emp.id, liv.titulo, us.LOGIN, "
                    + "emp.DATAEMPRESTIMO, emp.DATADEVOLUCAO"
                    + " from emprestimo emp"
            ...
            pstm = conn.prepareStatement(consultarDevolvidos);
            rs = pstm.executeQuery();

            while (rs.next()) {

                Livro livr = new Livro(null, rs.getString("titulo"), null, null, 0, 0);

                Usuario usuario = new Usuario(null, rs.getString("login"));

                Emprestimo e = new Emprestimo(rs.getLong("id"),
                        rs.getDate("dataemprestimo"), rs.getDate("datadevolucao"),
                        livr, usuario);
